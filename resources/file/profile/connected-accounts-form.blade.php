<x-action-section>
    <x-slot name="title">
        {{ __('startupful::connected-accounts.title') }}
    </x-slot>

    <x-slot name="description">
        {{ __('startupful::connected-accounts.description') }}
    </x-slot>

    <x-slot name="content">
        <div class="p-4 bg-red-500/10 text-red-500 border-l-4 border-red-600 rounded font-medium text-sm">
            {{ __('startupful::connected-accounts.warning') }}
        </div>
                            ...
                            <div class="flex items-center space-x-6">
                                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos() && ! is_null($account->avatar_path))
                                    <button class="cursor-pointer ms-6 text-sm text-gray-500 hover:text-gray-700 focus:outline-none" wire:click="setAvatarAsProfilePhoto({{ $account->id }})">
                                        {{ __('startupful::connected-accounts.use_avatar') }}
                                    </button>
                                @endif
                                ...
                                @if (($this->accounts->count() > 1 || ! is_null(auth()->user()->getAuthPassword())))
                                    <x-danger-button wire:click="confirmRemoveAccount({{ $account->id }})" wire:loading.attr="disabled">
                                        {{ __('startupful::connected-accounts.remove') }}
                                    </x-danger-button>
                                @endif
                            </div>
                        @else
                            <x-action-link href="{{ route('oauth.redirect', ['provider' => $provider['id']]) }}">
                                {{ __('startupful::connected-accounts.connect') }}
                            </x-action-link>
                        @endif
                    </x-slot>
        ...
        <x-dialog-modal wire:model="confirmingAccountRemoval">
            <x-slot name="title">
                {{ __('startupful::connected-accounts.confirm_removal_title') }}
            </x-slot>

            <x-slot name="content">
                {{ __('startupful::connected-accounts.confirm_removal_description') }}

                <div class="mt-4" x-data="{}" x-on:confirming-delete-user.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-input type="password" class="mt-1 block w-3/4"
                             autocomplete="current-password"
                             placeholder="{{ __('startupful::connected-accounts.password') }}"
                             x-ref="password"
                             wire:model="password"
